#' L-BFGS implementation of Advanced coupled matrix and tensor factorizations
#'
#' @inherit acmtf_opt
#' @inherit acmtfr_fun
...
#' model = acmtfr_lbfgs(Z, Y, 2, rel_tol=1e-5, abs_tol=1e-5)
acmtfr_lbfgs = function(Z, Y, numComponents, initialization="random", alpha=1, beta=rep(1e-3, length(Z$object)), epsilon=1e-8, pi=0.5, cg_update="HS", line_search="MT", max_iter=10000, max_fn=10000, abs_tol=1e-10, rel_tol=1e-10, grad_tol=1e-10, nstart=1, numCores=1, sortComponents=TRUE, allOutput=FALSE){
  ...
  if((numCores > 1) & (nstart > 1)){
    ...
    models = foreach::foreach(i=1:nstart) %dopar% {
      ...
      model = mize::mize(par=inits[[i]], fg=fg, max_iter=max_iter, max_fn=max_fn, abs_tol=abs_tol, rel_tol=rel_tol, grad_tol=grad_tol, method="L-BFGS", cg_update=cg_update, line_search=line_search)
    }
    ...
    for(i in 1:nstart){
      fg = list("fn"=function(x){return(CMTFtoolbox::acmtfr_fun(x,Z,Y,alpha,beta,epsilon,pi))}, "gr"=function(x){return(CMTFtoolbox::acmtfr_gradient(x,Z,Y,alpha,beta,epsilon,pi))}, "fg"=function(x){return(CMTFtoolbox::acmtfr_fg(x,Z,Y,alpha,beta,epsilon,pi))})
      models[[i]] = mize::mize(par=inits[[i]], fg=fg, max_iter=max_iter, max_fn=max_fn, abs_tol=abs_tol, rel_tol=rel_tol, grad_tol=grad_tol, method="L-BFGS", cg_update=cg_update, line_search=line_search)
    }
  }
