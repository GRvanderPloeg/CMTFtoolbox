...
FMS_to_simulation = function(inputFac, Fac, modes){
  ...
  for(i in 1:numComponents){
    ...
    for(j in 1:numModes){
      vect1 = inputFac[[j]][,i]
      vect2 = Fac[[j]][,i]
      norm_vect1 = norm(as.matrix(vect1),"F")
      norm_vect2 = norm(as.matrix(vect2), "F")
      
      ksi_input = ksi_input * norm_vect1
      ksi_model = ksi_model * norm_vect2
      
      vect1_normalized = vect1 / norm_vect1
      vect2_normalized = vect2 / norm_vect2
      
      FMS = FMS * (t(vect1_normalized) %*% vect2_normalized)
    }
    FMS_result[i] = (1 - ((abs(ksi_input - ksi_model)) / max(ksi_input, ksi_model))) * abs(FMS)
  }
  return(min(FMS_result))
}

inputFac = list(A,B,C,D)
Fac = vect_to_fac(CMTF_010[[1]]$par, Z_010)
FMS_to_simulation(inputFac, Fac, Z_010$modes)
```
```{r small ksi test}
